...
func parseSQLResponse(resp *pb.SQLResponse) any {
	if resp.GetError() != nil {
		return resultError{resp.GetError()}
	}

	if resp.GetTupleFormat() == nil {
		return resultType{}
	}

	res := resultType{
		names: slices.Clone(resp.GetTupleFormat().GetNames()),
		rows:  make([]resultRow, len(resp.GetTuples())),
	}
	for i := range resp.GetTuples() {
		res.rows[i] = make([]any, 0, len(resp.GetTupleFormat().GetNames()))
	}

	for r, row := range resp.GetTuples() {
