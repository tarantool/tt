...
type TemplateManifest struct {
	...
	Vars []UserPrompt
	// PreHook - path to the executable to run before template instantiation.
	PreHook string `mapstructure:"pre-hook"`
	// PreHook path to the executable to run after template instantiation.
	PostHook string `mapstructure:"post-hook"`
}

func GetManifest(manifestPath string) (TemplateManifest, error) {
