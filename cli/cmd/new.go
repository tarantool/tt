...
func NewNewCmd() *cobra.Command {
	newCmd := &cobra.Command{
		...
		Long: "Write the manifest a tt package is described by: the format " +
			"version, the package name (taken from the directory name), the " +
			"Tarantool and tt version requirements and an empty [dependencies] " +
			"table for tt package add to write into. Components and products are " +
			"left as commented examples, since their contents depend on a layout " +
			"only the author knows. An existing app.manifest.toml is never " +
