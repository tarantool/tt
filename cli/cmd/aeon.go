...
var aeonConnectCtx = aeon.ConnectCtx{
	TransportMode: aeon.AeonTransportDefault,
}
...
func aeonTransportCompletion(cmd *cobra.Command, args []string, toComplete string) (
	...
	suggest := make([]string, 0, len(aeon.AeonValidTransport))
	for k, v := range aeon.AeonValidTransport {
...
func aeonConnectValidateArgs(cmd *cobra.Command, args []string) error {
	if aeonConnectCtx.TransportMode == "ssl" {
	...
	aeonConnectCtx.Username = os.Getenv(libconnect.TarantoolUsernameEnv)
	aeonConnectCtx.Password = os.Getenv(libconnect.TarantoolPasswordEnv)
