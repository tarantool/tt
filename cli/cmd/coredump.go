...
func NewCoredumpCmd() *cobra.Command {
	...
	var packCmd = setupTtModuleCmd(internalCoredumpPackModule, &cobra.Command{
		Use:   "pack COREDUMP",
	...
	var unpackCmd = setupTtModuleCmd(internalCoredumpUnpackModule, &cobra.Command{
		Use:   "unpack ARCHIVE",
		Short: "unpack tarantool coredump tar.gz archive",
		Args:  cobra.ExactArgs(1),
	})

	var inspectCmd = setupTtModuleCmd(internalCoredumpInspectModule, &cobra.Command{
