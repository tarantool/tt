...
// devScope is what the dev closure is called in the human table's first column.
// It is not a product — the manifest declares dev dependencies globally — so it
// is spelled differently from every product name, which the name rules make
...
// Render writes a report in the chosen format.
func Render(out io.Writer, report *Report, format Format) error {
	switch format {
	case FormatJSON:
...
//
// The lock state is a line above the table rather than a column, because it
// qualifies every version below it at once: a stale lock means the whole
// VERSION column is what the last resolution chose, not what the next one will.
// A manifest that declares nothing gets a sentence instead of a bare header,
// which would read like a bug.
func renderTable(out io.Writer, report *Report) error {
	err := renderLockLine(out, report)
	if err != nil {
		return err
	}
	...
	_, err = fmt.Fprintln(table, "SCOPE\tNAME\tCONSTRAINT\tVERSION\tSOURCE\tKIND")
	if err != nil {
		return fmt.Errorf("rendering table: %w", err)
	}
	...
	for _, row := range rows {
		_, err = fmt.Fprintf(table, "%s\t%s\t%s\t%s\t%s\t%s\n",
			dash(row.scope), row.entry.Name, dash(row.entry.Constraint),
			dash(row.entry.Version), dash(row.entry.Source), kindOf(row.entry))
		if err != nil {
			return fmt.Errorf("rendering table: %w", err)
		}
	}
...
func tableRows(report *Report) []tableRow {
	...
	for _, product := range report.Products {
		for _, entry := range product.Dependencies {
			rows = append(rows, tableRow{scope: product.Name, entry: entry})
		}
	}

	for _, entry := range report.DevDependencies {
		rows = append(rows, tableRow{scope: devScope, entry: entry})
	}

	return rows
}

// kindOf labels a row as declared by the manifest or pulled in behind a
// declaration.
