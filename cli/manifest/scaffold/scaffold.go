...
// fallbackPackageName is used when the directory name cannot be turned into a
// valid package name. It is always valid, so tt new never fails over something
// the user can fix in one edit of the file it just wrote.
const fallbackPackageName = "app"
...
type Options struct {
	...
	// be absolute.
	ProjectDir string
	// Warn receives non-fatal diagnostics — notably a directory name that could
	// not be used as a package name. Nil drops them.
	Warn func(string)
}
...
//
// The package name is derived from the directory name, which is what a user
// means by "this project" often enough to be the right default and is trivially
// editable when it is not. A directory whose name cannot be a package name —
// it starts with a digit, it is one of the reserved names, it is all
// punctuation — falls back to "app" with a warning rather than failing: the
// command's job is to produce a valid manifest, and a name is one line to
// change in the file it just wrote.
//
...
// packageNameFor derives the package name from the project directory, warning
// when the directory name could not be used.
func packageNameFor(opts Options) string {
	...
	if opts.Warn != nil {
		opts.Warn(fmt.Sprintf(
			"cannot use directory name %q as a package name; using %q — edit [package].name",
			dirName, fallbackPackageName))
	}

	return fallbackPackageName
}

// normalizeName turns a directory name into a package name, or returns "" when
// it cannot.
//
...
// afterwards, or that is one of the reserved names, is rejected rather than
// mangled further: "2do" could become "app-2do", but inventing a prefix is a
// worse answer than saying the name could not be used.
