...
func checkDefaultEnv(t *testing.T, configName string, instancesEnabled string) {
	...
	assert.Equal(t, instancesEnabled, cfg.CliConfig.Env.InstancesEnabled)
	...
	assert.Equal(t, 10, cfg.CliConfig.Env.LogMaxBackups)
	assert.Equal(t, 100, cfg.CliConfig.Env.LogMaxSize)
	assert.Equal(t, 8, cfg.CliConfig.Env.LogMaxAge)
	assert.Equal(t, "bin", cfg.CliConfig.Env.BinDir)
	assert.Equal(t, "modules", cfg.CliConfig.Modules.Directory)
	assert.Equal(t, "distfiles", cfg.CliConfig.Repo.Install)
	assert.Equal(t, "include", cfg.CliConfig.Env.IncludeDir)
	assert.Equal(t, "templates", cfg.CliConfig.Templates[0].Path)
	assert.False(t, cfg.CliConfig.Env.TarantoolctlLayout)
...
func TestGenerateTtEnv(t *testing.T) {
	...
	assert.Equal(t, ".", cfg.CliConfig.Env.InstancesEnabled)
...
func TestInitRunTarantoolctlCfg(t *testing.T) {
	...
	assert.Equal(t, "./instances", cfg.CliConfig.Env.InstancesEnabled)
	...
	assert.True(t, cfg.CliConfig.Env.TarantoolctlLayout)
	assert.DirExists(t, cfg.CliConfig.Env.InstancesEnabled)
	assert.DirExists(t, "modules")
	assert.DirExists(t, "include")
	assert.DirExists(t, "bin")
	assert.DirExists(t, "distfiles")
