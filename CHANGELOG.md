...
- `tt new`: create a skeleton `app.manifest.toml` in the current directory —
  the format version, the package name taken from the directory name, the
  Tarantool and tt requirements and an empty `[dependencies]` table for
  `tt package add` to write into. Components and products are left as commented
  examples, since their contents depend on a layout only the author knows. An
  existing manifest is never overwritten: the command fails instead. This is
  not `tt create`, which lays out a whole application from a template.
