...
def test_downgrade_t2_app_dummy_replicaset(tt_cmd):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            downgrade_cmd = [
                tt_cmd, "replicaset", "downgrade", app_name, "--custom", "2.8.2"
            ]
...
def test_cluster_replicasets(tt_cmd, tmp_path):
    ...
    try:
        ...
        downgrade_cmd = [tt_cmd, "replicaset", "downgrade", app_name, "-t=15", "2.11.1"]
...
def test_downgrade_invalid_version(tt_cmd, tmp_path):
    ...
    try:
        ...
        downgrade_cmd = [tt_cmd, "replicaset", "downgrade", app_name, "-t=15", "1.1.1"]
        rc, out = run_command_and_get_output(downgrade_cmd, cwd=tmp_path)

        assert rc == 1
        assert "Version '1.1.1' is not allowed." in out

        downgrade_cmd = [tt_cmd, "replicaset", "downgrade", app_name, "-t=15", "3.0"]
        rc, out = run_command_and_get_output(downgrade_cmd, cwd=tmp_path)
...
def test_downgrade_remote_replicasets(tt_cmd, tmpdir_with_cfg):
    ...
    try:
        ...
        upgrade_cmd = [tt_cmd, "replicaset", "downgrade", uri, "-t=15", "2.11.1"]
...
def test_downgrade_cartridge(tt_cmd, cartridge_app):
    ...
    upgrade_cmd = [tt_cmd, "replicaset", "downgrade", cartridge_name, "-t=15", "2.10.0"]
