...
def make_test_failovers_param(
    ...
    return pytest.param(key, replicaset, inst, is_uri, stop_inst, err_text, args, id=id)
...
def test_promote_cconfig_failovers(
    ...
    instances = list(
        filter(
            lambda x: x.startswith(replicaset),
            [
                "off-failover-1",
                "off-failover-2",
                "manual-failover-1",
...
@pytest.mark.skipif(
    tarantool_major_version >= 3 or tarantool_major_version < 2,
    reason="skip cartridge tests for Tarantool 3.0 | " + "Tarantool < 2 does not support raft",
)
