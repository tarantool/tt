...
def test_new_writes_a_manifest_the_pipeline_accepts(tree, run_tt):
    ...
    # The package name comes from the directory, normalized to the name shape.
    assert parsed["package"]["name"] == root.name.lower().replace("_", "-")
...
def test_new_refuses_to_overwrite(tree, run_tt):
    ...
    assert run_tt("new").returncode == 0
