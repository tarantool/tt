...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["http://localhost:3303", "role"],
            "need to provide flag(s) with scope roles will removed",
        ),
    ],
)
...
def test_cluster_rs_roles_add_auth(
    tt_cmd,
    tmpdir_with_cfg,
    auth,
    instance_name,
...
@pytest.mark.parametrize(
    "instance_name, flags, cfg, expected_cfg, err_msg",
    [
        (
            "etcd",
            ["-G"],
...
""",
        ...
        (
            "etcd",
            ["-G", "-g", "group-001", "-r", "replicaset-001", "-i", "instance-001"],
            valid_cluster_cfg
            + """\
            roles:
              - config.storage
