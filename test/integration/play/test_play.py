...
from utils import (TarantoolTestInstance, control_socket, create_tt_config,
                   initial_snap, initial_xlog, lib_path,
...
def test_play_to_single_instance_app(tt_cmd, tmp_path):
    ...
    try:
        ...
        rc, output = try_execute_on_instance(tt_cmd, tmp_path, "test_app", test_app_path)
        assert rc

        test_cases = ("test_app", "test_app:instance001")
        for case in test_cases:
            cmd = [tt_cmd, "play", f"{case}", test_xlog_name]
            rc, output = run_command_and_get_output(cmd, cwd=tmp_path)
            assert rc == 0

        invalid_cases = ("test_app:unexisted")
        for case in invalid_cases:
            cmd = [tt_cmd, "play", f"{case}", test_xlog_name]
            rc, cmd_output = run_command_and_get_output(cmd, cwd=tmp_path)
            assert rc == 1
            assert "could not parse application name:" in cmd_output
