...
from utils import (log_file, log_path, run_command_and_get_output,
                   wait_string_in_file)
...
class VshardCluster:
    def __init__(self, tt_cmd, env_dir: Path, app_name: str) -> None:
        ...
        rc, out = run_command_and_get_output([tt_cmd,
                                              "create", "vshard_cluster",
                                              "--name", self.app_name,
                                              "-s", "-f"], cwd=self.env_dir)
        assert rc == 0

    def build(self):
        rc, out = run_command_and_get_output([self.tt_cmd, "build",
                                              self.app_name], cwd=self.env_dir)
        assert rc == 0
    ...
    def start(self):
        ...
        wait_string_in_file(self.env_dir / self.app_name / log_path / "router-001-a" / log_file,
                            "All replicas are ok")

        for inst in ["storage-001-a", "storage-002-a"]:
            wait_string_in_file(self.env_dir / self.app_name / log_path / inst / log_file,
                                "leaving orphan mode")
